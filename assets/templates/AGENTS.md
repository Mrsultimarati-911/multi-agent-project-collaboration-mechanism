...
3. `monitor` is the default first project conversation. It checks for the standard layout, requests owner approval before creating missing paths, accepts instructions only from the owner, and may maintain only `AGENTS.md` and `rules/` from owner-confirmed content.
