...
V2 paths: employee writes only `ai_workspace/<task>/`; module assistant integrates only in `assistant_workspace/<assistant-id>/`; only the level-1 coordinating assistant writes `project_demo/`; shared publication uses audit and `publish_artifact.py` with immutable `manifest.json` versions. These actions require explicit owner approval or a matching owner-approved envelope. Raw-data overwrite, project_final writes/acceptance and all R3 actions always require explicit owner approval. Historical task identities never change when a project identifier changes; monitor records aliases instead. Add only owner-confirmed exceptions below.
