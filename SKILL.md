...
The human project owner approves Level 1 objectives, the Authority Envelope, material changes, exceptions and final acceptance. Level 2 dispatch is either `owner-approved` or `envelope-authorized` by that matching owner-approved envelope. An approval can authorize a bounded class of actions; never infer approval from discussion, likely preferences or an agent recommendation.
...
3. Ask the owner for the following initialization values: required global task-code project identifier; optional `record`, `assistant`, and `employee` default model/reasoning; optional work language; optional project-specific rules; and Feishu notifications, enabled by default. See [startup protocol](references/startup-protocol.md).
...
Before an assistant submits the first approved stage plan, read [level-1 plan record](references/level1-plan-record.md) and use `assets/templates/root/templates/LEVEL1_PLAN.md`. A `level1_plan` is the frozen contract preceding dispatch. A changed authority/scope requires a newly authorized plan with a new identity and `supersedes_reference`; appended discussion never mutates the old envelope or task state.
...
For execution, exception, completion, or stage-close records, read [remaining work-record types](references/remaining-work-record-types.md) and use the matching template. Record submits JSON events to the engine, which alone generates machine-readable Markdown metadata. `level2_results_mid` preserves every attempt; warning and error preserve interruption history; final results and summaries are new records, never renames of earlier ones.
...
Record prepares factual events in the configured language; `record_engine.py` determines structural, state, dependency, reference and append-only legality from authoritative history using the same schema as the validator. Invoke `notifications/feishu.py` for governance events with `owner_intervention_required: true`, plus the explicitly authorized startup connectivity check. It reads secrets exclusively from environment variables, deduplicates unresolved events and never treats Feishu as approval. Project JSON contains only a non-sensitive `enabled` boolean.
...
- `scripts/governance_schema.py`: canonical record types, fields, task identities, states and path validation shared by engine and validator.
...
- Monitor may bootstrap `record|工作记录` and `assistant_00` after required initialization. Assistants may create and dispatch employees with either explicit owner approval or a matching owner-approved Authority Envelope. Bounded retries, corrections, employee replacement and permitted sharing/integration use that existing authority; R3 actions and envelope breaches require explicit owner approval.
...
- Do not add project-specific facts, credentials, private data, or domain-specific implementation rules to this Skill. Suggested model defaults belong to the reusable workflow; owner-specific overrides belong to project rules.
